...
// text를 angujerjs로 컴파일 하여 적용
    ...
    .directive('scalableProgressBar', ['$timeout', function ($timeout) {
        var templateHtml = '<div class="progress-striped progress"  style="width:{{swidth}}%">\n'
            ...
            + '</div>\n'
            + '<div class="progress-bar percent" style="background:bottom;width:100%;top:-100%;" role="progressbar">\n'
        ...
        return {
            ...
            link: function (scope, element, attrs){
                ...
                var start = function () {
                    ...
                    if (base > 0) {
                        defaultPercent = parseInt(base * 100 / max, 10);
                        checkPercent += defaultPercent;
                    }
                    var addPercent = 0;
                    if (scalable > 0) {
                        addPercent = parseInt(scalable * 100 / max, 10);
                        checkPercent += addPercent;
                        if((addPercent+defaultPercent) > 100) addPercent = 100-defaultPercent;
                    }
                    scope.percent = defaultPercent + addPercent;
